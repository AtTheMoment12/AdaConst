...
def L1_projection(x2, y2, eps1):
    ...
    y2: current perturbation (x2 + y2 is the point to be projected)--noise
...
class APGDAttack():
    ...
    def __init__(
        ...
        assert self.norm in ['Linf', 'L2', 'L1', 'L1mask']
    ...
    def normalize(self, x):
        ...
        elif self.norm == 'L1' or self.norm == 'L1mask':
            try:
    ...
    def attack_single_run(self, x, y, x_init=None):
        ...
        if not x_init is None:
            x_adv = x_init.clone()
            if self.norm == 'L1' or self.norm == 'L1mask' and self.verbose:
        ...
        self.mask = torch.where(
            grad.abs() > torch.kthvalue(grad.abs().view(grad.shape[0], -1),
                                        k=int(3 * 299 * 299 * (1 - (self.eps * 255) ** 0.5 / 5432)), dim=-1)[
                             0].type_as(grad)[:, None, None, None], 1, 0)
        ...
        alpha = 2. if self.norm in ['Linf', 'L2'] else 1. if self.norm in ['L1','L1mask'] else 2e-2
        step_size = alpha * self.eps * torch.ones([x.shape[0], *(
            [1] * self.ndims)]).to(self.device).detach()
        x_adv_old = x_adv.clone()
        counter = 0
        k = self.n_iter_2 + 0
        n_fts = torch.numel(x[0])
        if self.norm == 'L1' or self.norm == 'L1mask':
        ...
        for i in range(self.n_iter):
            ...
            with torch.no_grad():
                ...
                elif self.norm == 'L1mask':
                    grad_topk = grad.abs().view(x.shape[0], -1).sort(-1)[0]
                    topk_curr = torch.clamp((1. - topk) * n_fts, min=0, max=n_fts - 1).long()
                    grad_topk = grad_topk[u, topk_curr].view(-1, *[1] * (len(x.shape) - 1))
                    sparsegrad = grad * (grad.abs() >= grad_topk).float()
                    x_adv_1 = x_adv + step_size * sparsegrad.sign()*self.mask / (
                            L1_norm(sparsegrad.sign()*self.mask, keepdim=True) + 1e-10)
