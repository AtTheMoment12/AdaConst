...
class MIFGSML1(Attack):
    ...
    Examples::
        >>> attack = torchattacks.MIFGSML1(model, eps=8/255, steps=10, decay=1.0)
        >>> adv_images = attack(images, labels)

    """

    def __init__(self, model, eps=100000 / 255, alpha=2 / 255, steps=10, decay=1.0):
        super().__init__("MIFGSML1", model)
        self.eps = eps
        self.steps = steps
        self.decay = decay
        self.alpha = alpha
    ...
    def forward(self, images, labels):
        ...
        mask = torch.zeros_like(images).detach().to(self.device)
        ...
        for step in range(self.steps):
            ...
            delta = project_l1ball(noises.view( noises.size(0),-1), z=self.eps, axis=-1)
